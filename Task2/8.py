...
INPUT = os.path.join(BASE, "input", "8.jpeg")
OUTPUT_DIR = os.path.join(BASE, "output")
OUTPUT = os.path.join(OUTPUT_DIR, "8.jpeg")
...
def main():
    ...
    # Y-bounds tailored to match the span of the red dashed lines in 8.jpeg
    y_top = int(h * 0.52)
    y_mid = int(h * 0.72)
    y_bot = int(h * 0.80)

    # 1. Left boundary curve (Tracking the left lane divider dashed line marked in red)
    left_control = [
        [w * 0.53, y_top],   # Vanishing point near center
        [w * 0.43, y_mid],   # Mid point
        [w * 0.39, y_bot]    # Bottom-left edge
    ]

    # 2. Right boundary curve (Tracking the right shoulder white line marked in red)
    right_control = [
        [w * 0.56, y_top],   # Vanishing point near center
        [w * 0.72, y_mid],   # Mid point
        [w * 0.80, y_bot]    # Bottom-right edge
    ]
    ...
    print("Lane detection for 8.jpeg complete. Saved to:", OUTPUT)
