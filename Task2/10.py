...
INPUT = os.path.join(BASE, "input", "10.jpeg")
OUTPUT_DIR = os.path.join(BASE, "output")
OUTPUT = os.path.join(OUTPUT_DIR, "10.jpeg")
...
def generate_curve(control_points, num_points=50):
    """Fits a quadratic polynomial through control points (x, y) to match the straight road perspective."""
...
def main():
    ...
    # Y-bounds tailored to match the straight road perspective in 10.jpeg
    y_top = int(h * 0.55)
    y_mid = int(h * 0.60)
    y_bot = int(h * 0.90)

    # 1. Left boundary curve (Tracking the dashed white lane markings)
    left_control = [
        [w * 0.33, y_top],   # Vanishing point apex near center-left
        [w * 0.15, y_mid],   # Mid section
        [w * -0.2, y_bot]    # Bottom-left edge
    ]

    # 2. Right boundary curve (Tracking the solid white shoulder edge)
    right_control = [
        [w * 0.70, y_top],   # Vanishing point apex
        [w * 0.76, y_mid],   # Mid section tracking the white shoulder line
        [w * 1.22, y_bot]    # Bottom-right edge
    ]
    ...
    print("Lane detection for 10.jpeg complete. Saved to:", OUTPUT)
