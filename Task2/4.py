...
INPUT = os.path.join(BASE, "input", "4.jpeg")
OUTPUT_DIR = os.path.join(BASE, "output")
OUTPUT = os.path.join(OUTPUT_DIR, "4.jpeg")
...
def main():
    ...
    # Y-bounds tailored specifically for 4.jpeg perspective inside the tunnel
    y_top = int(h * 0.55)
    y_mid = int(h * 0.72)
    y_bot = int(h * 1.00)

    # 1. Left boundary curve (Dashed center lane markings)
    left_control = [
        [w * 0.455, y_top],   # Vanishing point near center
        [w * 0.37, y_mid],   # Mid-section
        [w * 0.18, y_bot]    # Bottom-left edge
    ]

    # 2. Right boundary curve (Solid white shoulder line)
    right_control = [
        [w * 0.490, y_top],   # Vanishing point near center
        [w * 0.60, y_mid],   # Mid-section
        [w * 0.720, y_bot]    # Bottom-right edge
    ]
    ...
    print("Lane detection for 4.jpeg complete. Saved to:", OUTPUT)
