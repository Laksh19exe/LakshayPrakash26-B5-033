...
INPUT = os.path.join(BASE, "input", "6.jpeg")
OUTPUT_DIR = os.path.join(BASE, "output")
OUTPUT = os.path.join(OUTPUT_DIR, "6.jpeg")
...
def main():
    ...
    # Y-bounds tailored specifically to match the span of the user's red lines
    y_top = int(h * 0.65)
    y_mid = int(h * 0.82)
    y_bot = int(h * 1.00)

    # 1. Left boundary curve (Following the dashed white lane marker drawn in red)
    left_control = [
        [w * 0.43, y_top],   # Top point matching the upper red line curve
        [w * 0.40, y_mid],   # Mid point
        [w * 0.38, y_bot]    # Bottom point
    ]

    # 2. Right boundary curve (Following the solid white shoulder line drawn in red)
    right_control = [
        [w * 0.46, y_top],   # Top point near the separation fork
        [w * 0.65, y_mid],   # Mid point tracking the outer shoulder curve
        [w * 0.90, y_bot]    # Bottom point expanding to the edge
    ]
    ...
    print("Lane detection custom-mapped to edited-image.jpg complete. Saved to:", OUTPUT)
